...
| R-004 | Auto Material capture orientation, color response, intrinsic emission, or trial cleanup differs in the live scene. | Exact pass/camera contracts, E0/endpoint/SPSA transaction, source-hit surface gate, byte-verified restore, strict accepted/fallback publication. | Supported-body live capture session with redacted readback evidence and bounded frame cost; maintainer + agent-operated host. | `BLOCKED_EXTERNAL_VALIDATION` |
